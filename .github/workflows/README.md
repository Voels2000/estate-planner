...
| **`e2e-smoke.yml`** | **`e2e-smoke`** (aggregator) | PR → `main`, `staging`; `workflow_dispatch` | Staging Supabase + Playwright users; **parallel** suite jobs: `e2e-go-live-profile`, `e2e-security-smoke`, `e2e-b4-gate`, `e2e-security-isolation` (shared `e2e-prepare` seed+build). Branch protection on **`main`** still requires check name **`e2e-smoke`**. |
