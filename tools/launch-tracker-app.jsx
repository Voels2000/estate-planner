...
const STORAGE_KEY = "mwm-launch-tracker-v1";

const SECTIONS = [
  {
    id: "done",
    title: "Verified & checked",
    note: "Closed by Cursor in Prompts 1–2. Kept here for context.",
    accent: C.ready,
    items: [
      ...
      { id: "b7-protected", label: "B7 PROTECTED list confirmed (avoels, stephen.a.voels, david…)", cmd: "cleanup-test-accounts.ts:70-95", type: "done" },
      { id: "b7-guard", label: "B7 prod-URL guard live (refuses prod without force)", cmd: "cleanup-test-accounts.ts:35-55 / :416", type: "done" },
      { id: "sec-smoke", label: "Security hardening manual checks 4/4", type: "done" },
      { id: "b8-eng", label: "B8 engineering gates (5 items)", type: "done" },
      { id: "b6-placeholders", label: "Legal placeholders wired (/terms, /privacy)", cmd: "lib/legal/company.ts", type: "done" },
    ],
  },
  {
    id: "b1",
    title: "B1 · Redeploy + automated smoke",
    note: "Run first. preflight + go-live-profile failed locally (auth timeout) — re-run after redeploy.",
    accent: C.ink,
    items: [
      { id: "b1-redeploy", label: "Redeploy Vercel with latest main", type: "open", attest: true },
      { id: "b1-preflight", label: "release:preflight green", cmd: "npm run release:preflight", type: "open" },
      { id: "b1-golive", label: "go-live-profile (17) — re-run, 0/17 last time", cmd: "npm run test:e2e:go-live-profile", type: "open" },
      { id: "b1-seciso", label: "security-isolation green", cmd: "npm run test:e2e:security-isolation", type: "open" },
      { id: "b1-xrole", label: "cross-role green", cmd: "npm run test:e2e:cross-role", type: "open" },
      { id: "b1-postdeploy", label: "post-deploy green", cmd: "npm run release:post-deploy", type: "open", attest: true },
      { id: "b1-prodsmoke", label: "prod smoke (42 @production)", cmd: "PLAYWRIGHT_BASE_URL=https://www.mywealthmaps.com npm run test:e2e:prod:smoke -- --workers=1", type: "open", attest: true },
    ],
  },
  {
    id: "b3",
  ...
  },
  {
    ...
    note: "Human smokes. Run before the B7 purge. Fresh-signup may only fully close at flip.",
    accent: C.ink,
    items: [
      ...
      { id: "b4-signup", label: "Fresh signup on prod URL", type: "open", attest: true, flag: "needs PUBLIC_SIGNUP_OPEN — may complete at flip, not before" },
    ],
  },
  {
    id: "b5",
    title: "B5 · Stripe live mode",
    note: "Largest gap. Code wired — Dashboard + Vercel config you must attest.",
    accent: C.wait,
    items: [
      ...
      { id: "b5-envls", label: "vercel env ls shows STRIPE_PRICE_* / _ATTORNEY_* / _ADVISOR_*", cmd: "vercel env ls production", type: "open", attest: true },
  ...
  {
    id: "b7",
    title: "B7 · Pre-flip cleanup",
    note: "Run LAST — immediately before flip, after every B4 smoke passes.",
    accent: C.flip,
    items: [
      { id: "b7-exec", label: "purge dry-run → purge → seed:e2e → compliance SQL", cmd: "verify .env.local target first", type: "deferred", attest: true },
    ],
  },
  {
    id: "a",
  ...
  },
  {
    ...
    accent: C.flip,
    items: [
      { id: "c-flip", label: "Auth ON → PUBLIC_SIGNUP_OPEN=true → MFA → redeploy → verify → post-deploy", type: "flip" },
    ],
  },
];
