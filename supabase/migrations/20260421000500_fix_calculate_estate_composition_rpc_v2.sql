-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: fix_calculate_estate_composition_rpc_v2
-- Session 27 / Sprint 85
-- Final column name corrections after schema inspection:
--   real_estate.property_name   → real_estate.name
--   businesses.business_name    → businesses.name
--   insurance_policies.owner_id → insurance_policies.user_id
-- Note: businesses already has valuation_discount_pct — we add dloc_pct/dlom_pct
--       as separate fields for multiplicative math. Both coexist safely.
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- 3. Financial assets
  -- assets.liquidity column already exists — use directly
  -- null liquidity treated as liquid (conservative default)
  for v_asset_rec in
    select
      a.id,
      a.name,
  ...
  -- Column is real_estate.name (not property_name)
  for v_asset_rec in
    select
      re.id,
      re.name,
  ...
  -- 5. Businesses — multiplicative DLOC/DLOM
  -- Column is businesses.name (not business_name)
  -- dloc_pct and dlom_pct added by migration 000000
  for v_asset_rec in
    select
      b.id,
      b.name,
  ...
  loop
    ...
    begin
      ...
      if v_asset_rec.estate_inclusion_status = 'included' then
        ...
        -- Multiplicative: combined = 1 - (1-dloc) * (1-dlom)
  ...
  -- 6. Insurance — death benefit
  -- insurance_policies uses user_id (not owner_id)
  -- is_ilit exists but exclusion flows through estate_inclusion_status
  for v_asset_rec in
    select
      ip.id,
      ip.policy_name,
      ip.death_benefit,
      ip.estate_inclusion_status
    from insurance_policies ip
    where ip.user_id = v_owner_id
  loop
    if v_asset_rec.estate_inclusion_status = 'included' then
  ...
  -- 9. Liabilities — non-mortgage (liabilities.balance) + mortgages from real_estate
  ...
  -- 11. Marital deduction — full gross at first death for MFJ (informational)
