-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: fix_calculate_estate_composition_rpc
-- Session 27 / Sprint 85
-- Fixes column references in calculate_estate_composition:
--   assets.asset_type → assets.type
--   assets.liquidity column already exists — use directly instead of computing
--   insurance_policies.is_ilit already exists — use for exclusion detection
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- 3. Financial assets
  -- Use the existing assets.liquidity column directly ('liquid' | 'illiquid' | null)
  -- Treat null liquidity as liquid (conservative default)
  for v_asset_rec in
    select
      a.id,
      a.name,
  ...
  -- 5. Businesses — multiplicative DLOC/DLOM discount on taxable value
  for v_asset_rec in
    select
  ...
  -- Use existing is_ilit column: when true and estate_inclusion_status is
  -- excluded_irrevocable, policy moves to outside bucket automatically via
  -- the estate_inclusion_status column (set by advisor in EstateTab).
  -- is_ilit alone does not exclude — the transfer must be marked complete.
  for v_asset_rec in
  ...
  -- 9. Liabilities — non-mortgage + mortgage balances from real_estate
