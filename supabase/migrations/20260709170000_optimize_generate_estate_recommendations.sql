-- Dedupe calculate_estate_composition in recompute: accept precomputed composition JSONB
-- and read state tax from composition (drops redundant calculate_state_estate_tax call).

CREATE OR REPLACE FUNCTION generate_estate_recommendations(
  p_household_id UUID,
...
BEGIN
  ...
  IF p_composition IS NOT NULL THEN
    v_composition := p_composition;
  ELSE
    SELECT calculate_estate_composition(p_household_id, 'consumer', v_gift_total) INTO v_composition;
  END IF;

  v_gross_estate := COALESCE((v_composition->>'gross_estate')::NUMERIC, 0);
  v_federal_tax := COALESCE(
    (v_composition->>'estimated_tax_federal')::NUMERIC,
    COALESCE((v_composition->>'estimated_tax')::NUMERIC, 0)
      - COALESCE((v_composition->>'estimated_tax_state')::NUMERIC, 0),
    0
  );
  v_taxable_federal := COALESCE((v_composition->>'taxable_estate')::NUMERIC, 0);
  v_state_tax := COALESCE((v_composition->>'estimated_tax_state')::NUMERIC, 0);
  v_taxable_state := CASE
    WHEN v_state_tax > 0 THEN GREATEST(0, v_gross_estate - v_state_exemption)
    ELSE 0
  END;
