...
export function buildAttorneyDirectoryOutreachEmail(fields: {
  ...
  const paragraphs = [
    `Hi ${fields.firstName},`,
    `We've listed ${fields.firmName} on My Wealth Maps, a Washington estate-planning platform that connects attorneys with households actively working through their estate plans — trust funding, WA estate tax exposure, beneficiary coordination.`,
    ...
    `If ${fields.firmName} isn't the right contact, just reply and let us know who is.`,
  ...
  return {
    ...
    bodyHtml: paragraphsToHtml(paragraphs.slice(0, 5), {
      label: 'Claim your listing',
      href: fields.claimLink,
    }) + `<p style="margin:0;color:#9ca3af;font-size:12px">${paragraphs[5]}</p>`,
  }
}
...
export function buildAdvisorDirectoryOutreachEmail(fields: {
  ...
  const paragraphs = [
    `Hi ${fields.firstName},`,
    `We've listed ${fields.firmName} on My Wealth Maps. It's a platform built for advisors working with high-net-worth households ($2M–$30M) on estate and retirement planning — households can connect their advisor directly and share plan data with your review in mind.`,
    ...
    `Wrong contact at ${fields.firmName}? Just reply and point us to the right person.`,
    `${fields.senderName}<br>My Wealth Maps`,
  ]
  return {
    subject,
