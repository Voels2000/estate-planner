...
export type MonteCarloPrefillFields = {
  ...
  has_spouse: boolean
  p2_birth_year: number | null
  p2_current_age: number | null
  p2_retirement_age: number | null
  p2_life_expectancy: number | null
  p2_social_security_monthly: number | null
  p2_social_security_start_age: number | null
  current_portfolio: number | null
  monthly_contribution: number | null
  stocks_pct: number | null
  bonds_pct: number | null
...
export type MonteCarloPrefillSummary = {
...
export type MonteCarloPrefillPayload = {
  person1_name: string | null
  person2_name: string | null
  prefill: MonteCarloPrefillFields
  confidence: Record<string, string>
  summary: MonteCarloPrefillSummary
}
...
export async function loadMonteCarloPrefill(userId: string): Promise<MonteCarloPrefillPayload> {
  ...
  const [
    ...
    admin
      ...
      .eq('owner_id', userId)
      .single(),
    admin.from('assets').select('type, value').eq('owner_id', userId),
    admin.from('income').select('source, amount, ss_person, start_year, end_year').eq('owner_id', userId),
    admin.from('expenses').select('amount, start_year, end_year').eq('owner_id', userId),
  ])
