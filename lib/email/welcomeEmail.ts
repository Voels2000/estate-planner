...
export async function sendWelcomeEmail(email: string, firstName: string) {
  const { error: emailError } = await resend.emails.send({
    ...
    to: email,
    subject: 'Welcome to My Wealth Maps',
    html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:40px 20px">
          <h1 style="color:#1a1a2e;font-size:24px">Welcome to My Wealth Maps</h1>
          ...
          <div style="background:#f9fafb;border-radius:8px;padding:32px;margin:24px 0">
            ...
            <p style="color:#374151;font-size:16px;line-height:1.6">Your My Wealth Maps account is ready. You now have access to a complete suite of financial planning tools.</p>
