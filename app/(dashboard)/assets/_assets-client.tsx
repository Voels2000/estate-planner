...
export function AssetsClient({
  ...
  const { grouped, groupKeys } = useMemo(() => {
    const g = assets.reduce<Record<string, Asset[]>>((acc, asset) => {
      const key = asset.type || 'other'
      if (!acc[key]) acc[key] = []
      acc[key].push(asset)
      return acc
    }, {})
    const keys = Object.keys(g).sort((a, b) => getTypeLabel(a).localeCompare(getTypeLabel(b)))
    keys.forEach((key) => {
  ...
  function handleSave(saved: Asset) {
    setShowModal(false)
    setEditAsset(null)
    setAssets((prev) => {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12">
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[color:var(--mwm-navy)]">Assets</h1>
                          ...
                          <td className="px-4 py-3 text-right">
                            {confirmDeleteId === asset.id ? (
                              ...
                              <span className="inline-flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => { setEditAsset(asset); setShowModal(true) }} className="text-sm text-[color:var(--mwm-navy)] font-medium hover:text-[color:var(--mwm-navy)]">Edit</button>
