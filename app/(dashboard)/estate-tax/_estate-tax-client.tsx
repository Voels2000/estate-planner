...
export default function EstateTaxClient({
  ...
  const result =
    brackets.length > 0
      ? computeFederalEstateTax(grossEstate, totalLiabilities, trustsExcluded, filing, brackets)
      : null
  ...
  function openEditTrust(t: EstateTaxTrustRow) {
    setEditTrust(t)
    setShowTrustModal(true)
    setError(null)
  }
    ...
    <div className="mx-auto max-w-5xl px-4 py-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <SummaryCard
          ...
          value={result ? formatDollars(result.taxable_estate) : '—'}
          sub="After liabilities & excluded trusts"
        />
        <SummaryCard
          label="Federal Exemption Used"
          value={result ? formatDollars(result.exemption_used) : '—'}
          sub="Sheltered amount (reporting)"
        />
        <SummaryCard
          label="Net Estate Tax"
          value={result ? formatDollars(result.net_estate_tax) : '—'}
          sub={result && result.net_estate_tax > 0 ? 'Estimated federal transfer tax' : 'No estimated tax due'}
          highlight={
            !result ? undefined : result.net_estate_tax > 0 ? 'red' : 'green'
          }
        />
      </div>
        ...
        <div className="mt-2">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 pt-4 border-t border-neutral-200">
            ...
            <span className="text-base font-bold text-neutral-900 tabular-nums">
              {result ? formatDollars(result.taxable_estate) : '—'}
            </span>
          </div>
        </div>
      </section>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4 overflow-y-auto">
          ...
          <div className="flex items-center gap-2">
            <input
              id="isIrrevocable"
              type="checkbox"
              checked={isIrrevocable}
              onChange={(e) => setIsIrrevocable(e.target.checked)}
              className="h-4 w-4 rounded border-neutral-300"
            />
            <label htmlFor="isIrrevocable" className="text-sm text-neutral-700">
              Is irrevocable
            </label>
          </div>
          <div className="flex items-center gap-2">
            <input
              id="excludesFromEstate"
              type="checkbox"
              checked={excludesFromEstate}
              onChange={(e) => setExcludesFromEstate(e.target.checked)}
              className="h-4 w-4 rounded border-neutral-300"
            />
            <label htmlFor="excludesFromEstate" className="text-sm text-neutral-700">
