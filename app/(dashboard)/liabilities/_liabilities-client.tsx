...
export function LiabilitiesClient({
  ...
  const { grouped, groupKeys } = useMemo(() => {
    const g = liabilities.reduce<Record<string, Liability[]>>((acc, liability) => {
  ...
  function handleSave(saved: Liability) {
    setShowModal(false)
...
function LiabilityModal({ editLiability, liabilityTypes, person1Name, person2Name, onClose, onSave }: {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const saved = (await res.json()) as Liability
      onSave(saved)
    } catch (err) {
