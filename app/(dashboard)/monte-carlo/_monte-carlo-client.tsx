      ...
      <div className="relative w-36 h-36">
        <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
          ...
          <circle
            ...
            strokeDasharray={`${rate * 2.513} 251.3`}
            strokeLinecap="round"
          />
        </svg>
  ...
  return (
    <ResponsiveContainer width="100%" height={320}>
      <AreaChart data={data} margin={{ top: 10, right: 20, left: 20, bottom: 0 }}>
        ...
        <Tooltip
          formatter={(v, name) => [formatCurrency(Number(v ?? 0)), name as string]}
...
function NumInput({ value, onChange, prefix, step = 1000, min = 0 }: {
        ...
        <div className="lg:col-span-1 space-y-4">
            ...
            {STEPS.map((s, i) => (
              <button
                ...
                className={`flex-1 text-xs py-1.5 rounded-md font-medium transition-colors ${
                  step === s.key ? 'bg-white shadow text-indigo-600' : 'text-gray-500 hover:text-gray-700'
                }`}
              >
                {i + 1}
              </button>
          ...
          {step === 'assumptions' && (
            <div className="space-y-4">
              ...
              <Field label="Include RMDs" hint="Required Minimum Distributions at 73+">
                <div className="flex items-center gap-2 mt-1">
                  <input
                    type="checkbox"
                    checked={inputs.include_rmd}
                    onChange={e => set('include_rmd', e.target.checked)}
                    className="w-4 h-4 accent-indigo-600"
                  />
                  <span className="text-sm text-gray-600">Factor in RMDs starting at age 73</span>
                </div>
              </Field>
              <div className="flex gap-2">
            ...
            {history.map(run => (
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  <span className={`text-sm font-semibold ${run.success_rate >= 90 ? 'text-green-600' : run.success_rate >= 75 ? 'text-amber-600' : 'text-red-500'}`}>
