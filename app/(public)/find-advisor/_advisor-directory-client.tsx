...
export function PublicAdvisorDirectoryClient({
  advisors,
  allSpecializations,
  allCredentials,
  allStates,
  ...
  const nav = {
    background: '#0f1f3d', padding: '14px 32px', display: 'flex',
    ...
    <div style={{ minHeight: '100vh', background: '#fafaf8',
      ...
      <div style={{ background: 'linear-gradient(135deg, #0f1f3d 0%, #1a3460 100%)',
        ...
        <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.7)',
          ...
          Connect with verified financial advisors who specialize in estate,
      ...
      <div style={{ background: 'white', borderBottom: '1px solid #e2e8f0',
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto',
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 6,
            ...
            <input type="checkbox" checked={fiduciaryOnly}
          ...
          {(search || selectedState || selectedSpec || selectedCred || fiduciaryOnly || remoteOnly) && (
        ...
        {filtered.length === 0 ? (
          ...
          <div style={{ display: 'grid',
            ...
            {filtered.map(advisor => {
              const alreadySent = sentIds.includes(advisor.id) ||
                existingConnections.includes(advisor.id)
              return (
                <div key={advisor.id} style={{ background: 'white',
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between',
                    ...
                    <div>
                      <div style={{ fontFamily: 'Playfair Display, Georgia, serif',
                        ...
                        marginBottom: 2 }}>{advisor.firm_name}</div>
                  ...
                  <div style={{ fontSize: 13, color: '#718096' }}>
                    📍 {[advisor.city, advisor.state].filter(Boolean).join(', ') || 'Location not listed'}
                  ...
                  {advisor.credentials?.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: 4 }}>
                      {advisor.credentials.map(c => (
                        <span key={c} style={{ background: '#fdf6e3',
                  ...
                  <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' as const }}>
                    ...
                    {advisor.minimum_assets && (
                      <div style={{ fontSize: 12, color: '#718096' }}>
                        💰 Min. ${advisor.minimum_assets.toLocaleString()}
                      </div>
                    )}
                  </div>
      ...
      {modalAdvisor && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)',
          ...
          onClick={e => { if (e.target === e.currentTarget) setModalAdvisor(null) }}>
          <div style={{ background: 'white', borderRadius: 16, padding: '36px',
            maxWidth: 460, width: '100%',
            boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <h2 style={{ fontFamily: 'Playfair Display, Georgia, serif',
              fontSize: 20, fontWeight: 500, color: '#0f1f3d', marginBottom: 4 }}>
              Connect with {modalAdvisor.firm_name}
            </h2>
            <p style={{ fontSize: 13, color: '#718096', marginBottom: 20, lineHeight: 1.6 }}>
              Send a connection request. They&apos;ll be able to view your planning
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
              <button onClick={() => setModalAdvisor(null)}
