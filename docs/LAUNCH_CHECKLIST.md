...
- [x] **RLS policy fix (pre-launch)** — `20260527150000_prelaunch_rls_household_scope.sql`; GST writes via `/api/advisor/gst-entry`; verify `scripts/verify-loose-rls-policies.sql` on prod after `db push`
- [ ] **RLS policy audit sign-off** — Post-fix export to `docs/audits/rls-policies-post-fix-*.csv`; functional isolation smoke (two consumers + advisor/client) — see `docs/audits/README.md`
