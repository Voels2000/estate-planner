...
- [ ] **[SPRINT 9 HARD GATE]** Life event context on advisor connection — when advisor accepts
  a client who arrived via a life event page, `life_events` context must be visible in advisor
  portal Overview. Has been deferred since Sprint 4. Must ship in Sprint 9.
  - File: `app/api/advisor/accept-request/route.ts`
  - Verification: Supabase query on `life_events` + advisor portal spot-check
...
## Pre-launch state (current — end of Sprint 9 partial)

| Area | Status | Blocks launch? |
|------|--------|----------------|
| Crawlers | `app/robots.ts` permissive rules in repo (Sprint 9); verify on prod domain after deploy | Section 2 only |
| Email drip | All 24 slugs have custom sequences in code (Sprint 9); prod smoke test still required | Yes — Sprint 14 |
| Sitemap | `app/sitemap.ts` ready; sitemap line restored in `robots.ts` | Section 2 only |
...
| Life-event-on-connect | Not shipped — deferred since Sprint 4 | **Yes — Sprint 9 hard gate** |
| Digital Assets FEATURE_TIERS key | Missing — page uses `page.tsx` tier check only | Yes — Sprint 9 |
| NEXT_PUBLIC_SITE_URL legacy routes | Audit incomplete | Yes — Sprint 9 |
| Business succession | Commented out of sidebar — decision pending | Yes — Sprint 10 decision |
| Invite-your-advisor onboarding | Footer only — decision pending | Yes — Sprint 10 decision |
| A/B test decision criteria | Not defined — must be set by end of Sprint 10 | Yes — Sprint 12 |
| Persona alerts (business + RE) | Not shipped | Yes — Sprint 10 |
