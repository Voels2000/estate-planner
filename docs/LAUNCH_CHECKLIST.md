...
- [ ] **Post-deploy Voels verify** — `npm run verify:post-deploy-voels` after prod deploy. If **`mc-precompute`** fails, run **`npm run smoke:mc-voels`** first (repopulates `monte_carlo_results` for Voels base case). Daily cron `/api/cron/post-deploy-verify` (9:00 UTC) alerts via Vercel logs on failure.
