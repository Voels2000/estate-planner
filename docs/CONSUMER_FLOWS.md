...
| **Tier / gate** | Tier 1; **no** profile gate; shows `DashboardEmptyState` if no household; **`DashboardOnramp`** when `canUnlockDashboard` false — needs MVI profile + assets + income (`lib/dashboard/canUnlockDashboard.ts`, `onrampGate.ts`) |
...
| **E2E** | `tests/e2e/consumer/dashboard.spec.ts` · `golden-path-show-all-tools.spec.ts` (seed assets+income; `check-golden-path-onramp-gate.ts`) |
| **Key UI sections** | **Onramp (State 1):** profile/assets/income unlock checkboxes · Import · Guide · Manual (equal) · faded estate preview · **State 2:** net worth hero · unlock prompt · estate readiness tile + health-check nudge · Financial/Retirement collapsibles · **State 3:** tax hero · adaptive greeting · **`EstateReadinessCard`** (six subcategory labels + **`InfoTooltip`** explainers) · **`PriorityAlertCard`** · collapsed other alerts · checklist/tax snapshot · estate summary collapsible |
...
| **Read APIs / RPCs** | `strategy_line_items`, `projection_scenarios.outputs_s1_first`, `state_estate_tax_rules`, cached `estate_health_scores.recommendations` (bypass savings; live `generate_estate_recommendations` only on cold cache) |
...
| **Signup attribution** | `POST /api/auth/signup` → confirm-email or session (or `/waitlist` email capture when waitlist mode on) | `_signup-form.tsx` → `profiles` via `completeSignup` + `POST /api/analytics/funnel` (`account_created`); open_consumer: server sends Supabase signup confirmation mail (`sendSignupConfirmationEmail`); waitlist: `POST /api/email-capture` (`source: 'waitlist'`) |
...
| **Private beta signup** | `/signup?access=TOKEN&label=cohort` while waitlist on | `BETA_SIGNUP_TOKEN` (Vercel Production); middleware sets cookies; `_beta-signup-tracker.tsx` → `beta_signup_link_viewed`; `account_created` with `signup_source: beta_access_link` |
| **Funnel analytics** | All public funnel steps | `captureFunnelEvent()` → `POST /api/analytics/funnel` → `funnel_events`; admin **Funnel** tab — 30-day step counts, tier conversion, slug/referral tables, **Private Beta Signup Links** cohorts |
