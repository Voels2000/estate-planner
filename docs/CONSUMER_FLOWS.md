...
**Estate summary strip (stage 2–3, `_dashboard-client.tsx` + `EstateCalloutCard.tsx`):** Render order — greeting + **intro status row** (`DashboardIntroSection`: conflict severity pills + compact **Estate readiness {score}/100** pill on same line when `estateHealthScore` present; mid-page dismissible conflict banner removed 2026-05-30) → **`EstateSummaryHeroAndMetrics`**: (1) tax exposure hero (~$937K WA) → (2) four compact metric tiles → (3) optional **bypass trust alert** via **`afterMetrics`** when `parseBypassTrustSavings(initialRecommendations)` &gt; 0 (Alan: **$645,463** from RPC reason `by $645,463`) → then **`sm:grid-cols-2`** **`EstateExecutionChecklist`** + **`EstateTaxSnapshotPanel`**. **`EstateSummarySection`** (collapsible below Financial/Retirement): full readiness breakdown (component bars) · composition · **titling badges + link only** — Common Planning Topics removed (2026-05-30).
