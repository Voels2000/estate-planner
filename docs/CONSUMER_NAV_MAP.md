...
| `/import` | Import Data | 1 | CSV/XLSX bulk import (Tier 1 upload + commit; job history Tier 2+); upload step: `SupportedFormats` + persona/CSV template downloads above drop zone; F-2: preamble/header detection, multi-sheet Excel picker, inline review edit, duplicate skip/import-all, `ingestion_job_id` traceability; `POST /api/ingest` → review → `POST /api/import/commit`; `DELETE /api/import/jobs/[id]`; templates in `public/templates/` |
