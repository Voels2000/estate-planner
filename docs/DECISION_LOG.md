...
**Decision.** Migration `20260726130000_profiles_advisor_select_status_gate.sql` — add the same connected-status filter as household/asset policies. Regression: `tests/e2e/security/advisor-profiles-revocation-rls.spec.ts` (revoke link → advisor JWT → direct `profiles` SELECT must deny).

**Verification (2026-06-26):** Staging **RED** pre-migration — revoked advisor PostgREST read returned `Morgan Demo` / `e2e-advisor-client@…`. Policy SQL applied; **GREEN** (4/4 security project). Prod policy applied same day. Ledger version **`20260726130000`** (renamed — `20260726120000` is reserved for `one_time_purchases_refund_ack`).
