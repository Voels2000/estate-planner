...
- As of **2026-06-07**, titling list perf: **`buildTitlingLookups`** + memoized **`getTitlingWarnings`** (`lib/titling/`); memoized **`AssetTitlingCard`**; **`household_people`** fetched in parallel on `page.tsx`. Edit modals (**`TitlingModal`**, **`BeneficiaryModal`**, **`BeneficiaryGapModal`**) code-split via **`next/dynamic`** in `_titling-client.tsx`. Table virtualization deferred.
