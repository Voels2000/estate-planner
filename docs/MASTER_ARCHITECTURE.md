...
- **PDF narrative engine (2026-05-30):** `lib/export/narrativeEngine.ts` produces cover-page executive summary, tax callout (`clear` / `sunset_risk` / `exposed`), health trend, gifting bar, and themed enriched action items. Narrative inputs fetched in parallel via `lib/export/fetchNarrativePdfFields.ts` (`Promise.all` on trust docs, strategy configs, insurance, prior health score, `calculate_gifting_summary` RPC). `ExportPanel` renders via `generatePDFHTML(exportPdfData)` when export wiring is enabled. Meeting Prep tab surfaces top 3 open alerts above Export.
