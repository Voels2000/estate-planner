...
- **Tax term explainers (Domain 3):** `lib/estate/taxTermExplainers.ts` — `taxTermExplainer(key, ctx?)` for educational tooltips (not compliance footers). Wired: **`EstateReadinessCard`** score subcategories; **`EstateSummaryHeroAndMetrics`** tile labels (`statePrimary` only at hero); consumer **`/estate-tax`** summary + waterfall; advisor **`StateTaxPanel`** (`No portability` badge + Exemption header, `taxTermCtx` from `currentYearRow`). Pending: `/projections` chart legend/disclaimer · **`GiftingDashboard`** `annual_exclusion` / `superfunding`.
