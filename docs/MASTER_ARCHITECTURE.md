...
- **Tax term explainers (Domain 3):** `lib/estate/taxTermExplainers.ts` — `taxTermExplainer(key, ctx?)` for educational tooltips (not compliance footers). Wired: **`EstateReadinessCard`** score subcategories; **`EstateSummaryHeroAndMetrics`** tile labels (`federal_headroom`, `federal_exemption`, `state_exemption` with `statePrimary` only). Pending: `/estate-tax`, advisor **`StateTaxPanel`**.
