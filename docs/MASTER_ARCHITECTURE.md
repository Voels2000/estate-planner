...
- As of Session 127, **minimum viable profile gate** (`lib/estate/profileGate.ts`, `requireMinimumViableProfile`, **`requireHouseholdRecord`**): consumer pages redirect to `/profile?required=true&missing=...&from=...` when household row missing or when `person1_name`, `state_primary`, `filing_status`, or `person1_birth_year` is missing. **Friction reduction sprint (2026-05-27):** profile form collects essentials only; retirement/SS/longevity/deduction deferred to inline prompts on `/social-security` and `/scenarios` (`ProfileIncompleteInlinePrompt`). **Sprint F (2026-05-27):** health-check, social-security, digital-assets, attorney-access use `requireHouseholdRecord`; trust-strategy drops inline empty state in favor of shared redirect.
