...
- **PDF narrative engine (2026-05-30):** `lib/export/narrativeEngine.ts` — executive summary, tax callout, health trend, gifting bar, themed action items, **`dedupeActionItems()`**, **`enrichActionItems()`**. Payload via **`loadAdvisorExportWiring.ts`** + **`GET /api/advisor/meeting-prep-pdf/[clientId]?type=report`**. **Four-surface polish (2026-06-01):** **`lib/advisor/advisorBriefHelpers.ts`** shared by PDF export, meeting brief print (`?type=brief`, template `sprint-four-surface-polish-v2`), meeting prep tab/modal; PDF page 2 asset breakdown + health components; **`advisor_notes.note_type`**; brief print uses same enrichment context as export via `loadAdvisorExportWiringForClient`. **Print brief stat cards (2026-06-01):** `findAtDeathRow` for projected estate/tax; 3-card layout (health · tax at death · projected at death); alert enrichment uses current-year gross.
