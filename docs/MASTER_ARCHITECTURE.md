...
- As of **2026-05-30**, consumer **`/dashboard`**: intro row in **`DashboardIntroSection`** — conflict pills + compact readiness pill (`estateHealthScore.score`) on one line; mid-page dismissible conflict banner removed. Estate summary collapsible retains full readiness breakdown + titling badges; bypass trust alert in **`afterMetrics`**. **`FinancialSummarySection`** shows net worth + income/debt cards only — **`AssetAllocationSummary`** removed from dashboard (full allocation on **`/allocation`** via **`loadAssetAllocationData`**). **`/allocation`**: downstream note to Projections + Monte Carlo after save; expanded risk profile caption.
...
- Dashboard client route composition in `app/(dashboard)/_dashboard-client.tsx` now delegates rendering to feature components under `app/(dashboard)/_components/dashboard/*` (`DashboardIntroSection`, `FinancialSummarySection`, `RetirementSummarySection`, `EstateSummarySection`). **`FinancialSummarySection`** no longer embeds **`AssetAllocationSummary`** (2026-05-30); allocation data loads on **`/allocation`** only.
