...
| Dashboard onramp (2026-06-26) | `/dashboard` — `canUnlockDashboard()` via `shouldShowOnramp()`; unlock = MVI profile + assets + income (wizard/score not gates); `DashboardOnramp` — three unlock checkboxes + equal wizard/import/manual paths; audit `npm run audit:dashboard-gate` |
| Dashboard onramp (2026-05-30) | `guidedHref` via `resolveGuidedOnboardingHref()`; import card broker/Excel/CSV; `/dashboard` wizard-gate exempt (`wizardGateExemptPrefixes.ts`) |
...
5. **Tax, Domicile, Estate, Retirement** — Proactive alert banners for time-sensitive issues on each tab. **Estate:** liquidity hero, composition waterfall, conflict cards, missing-doc alert, beneficiary-by-account groups, toggled estate-flow diagram. **Retirement:** readiness hero from `YearRow[]`; SS survivor/breakeven via `loadSocialSecurityData`; Roth conversion via `runRothAnalysis`; withdrawal sequencing guide.
...
- As of **2026-05-29 (Sprint B)**, consumer **`/dashboard`** uses **`getDashboardState()`** (2 | 3 in client; 1 = **`DashboardOnramp`** in `page.tsx`). **State 2:** net worth hero, source breakdown, financial metrics, estate readiness `{n}/100` tile + `/health-check` nudge, amber estate-unlock prompt, **`SetupProgressCard`**. **State 3:** tax exposure hero; when **`estateHealthScore`** exists — adaptive greeting, **`EstateReadinessCard`** (six subcategory **`InfoTooltip`** explainers from **`SCORE_CATEGORY_EXPLAINERS`**), **`PriorityAlertCard`**, collapsed other alerts (not gated on **`planStage`**); checklist/tax snapshot grid remains **`sectionVisible(3)`**. **`EstateSummarySection`** below Financial/Retirement — estate composition only (no score block, no titling conflict badges). Data: `priorScore` + `openAlerts` in **`dashboard/_dashboard-body.tsx`**. Onramp gate: `canUnlockDashboard()` — MVI profile + assets + income (`lib/dashboard/canUnlockDashboard.ts`, `onrampGate.ts`).
...
- As of **2026-05-31**, **`ConsumerEstateFlowView`** (“What happens when I die?” on **`/my-estate-strategy`**) receives **`horizons={selectedHorizons}`** from **`_my-estate-strategy-client.tsx`**, builds **`horizonOverride`** (same pattern as advisor **`EstateFlowDiagram`**) and passes it to **`generateEstateFlow`** — display totals match **`buildStrategyHorizons`** / tax table (not a second internal projection lookup). Stale-fetch guard on tab switch; tabs stay visible during load; prominent **`graph.summary.gross_estate`** above asset tiles; horizon caption from local tab state. Asset category tiles remain today's holdings with projected-horizon context note.
...
- As of **2026-06-07**, titling list perf: **`buildTitlingLookups`** + memoized **`getTitlingWarnings`** (`lib/titling/`); memoized **`AssetTitlingCard`**; **`household_people`** fetched in parallel on `page.tsx`. Edit modals code-split via **`next/dynamic`**. Card lists virtualized with **`VirtualTitlingCardList`** + **`@tanstack/react-virtual`** when **≥ 20** rows per list (window scroll, dynamic row height).
...
Consumer and strategy writes call `afterHouseholdWrite` → `touchHousehold` (bumps `updated_at`, nulls `projection_inputs_hash`, **`invalidateDashboardBundle`**) → `triggerEstateHealthRecompute`, which POSTs to `/api/recompute-estate-health` with header `x-recompute-secret`. The route runs `computeEstateHealthScore`, `detectConflicts` (batch-resolves via **`resolve_household_alerts_batch`** inline `UPDATE`; batch-upserts via **`upsert_household_alerts_batch`**), then **gifting → composition ×2 → `generate_estate_recommendations(p_composition)`** (no nested `calculate_state_estate_tax`), and **upserts `estate_composition_cache`** for both `consumer` and `advisor` roles. **Sprint P-2:** dashboard reads recommendations from cache on load. **2026-06-11:** strategy/attorney planning surfaces + `PlanningGapsSection` refresh read `estate_health_scores.recommendations` (live RPC only on cold cache). **Post-launch perf:** composition via `getCachedComposition` (cache miss → live RPC until first recompute). **2026-06-12:** `/dashboard` uses **`loadDashboardBundle`** (60s TTL, ~22 parallel queries); MC read path uses **`loadScenarioMonteCarloWithStaleness`** (serve stale + updating banner). **`PATCH /api/households/[id]`** (`admin_expense_pct`) also calls `touchHousehold`. **`POST /api/import/commit`** calls `afterHouseholdWriteForOwner` after bulk insert. **Local E2E:** `E2E_SKIP_RECOMPUTE=true` in `.env.test` skips the recompute HTTP trigger (production unset). Advisor meeting-prep routes read `estate_composition_cache` with `source_role: consumer`. **Disk IO:** migrations `20260709150000`–`20260709180100` + `20260712120000`; monitor Supabase Infrastructure → Disk IO. **Go-live perf:** [NEXT_SESSION.md §5](./NEXT_SESSION.md#5-go-live-performance-audit-2026-06-11).
...
- Local UI components are split under `app/(dashboard)/projections/_components/*` — **`EstateOutlookChart.tsx`** renders P10–P90 SVG bands + optional amber state exemption reference line (**`React.memo`**)
- **`lib/household/getHouseholdForOwner.ts`** — cached full **`households.*`** fetch for **`/dashboard`** (dedupes within request)
- **`lib/gifting/perRecipientLimit.ts`** — annual exclusion constants; used by estate-tax, gifting dashboard, trust-strategy pages
- Estate-tax server page loads composition via **`getCachedComposition`** only (no parallel assets/RE/business list fetches)
...
- Dashboard client route composition in `app/(dashboard)/_dashboard-client.tsx` now delegates rendering to feature components under `app/(dashboard)/_components/dashboard/*` (`DashboardIntroSection`, `FinancialSummarySection`, `RetirementSummarySection`, `EstateSummarySection`). **`getDashboardState()`** (2026-05-30, `b71af63`) switches **State 2** (net worth hero) vs **State 3** (tax hero + estate strip); State 1 = **`DashboardOnramp`** in `page.tsx`. **`FinancialSummarySection`** no longer embeds **`AssetAllocationSummary`** (`7e8bf00`); allocation data loads on **`/allocation`** only.
...
- **Federal bracket unification (2026-06-06):** Same helpers power horizons (**`horizonSnapshots.ts`**), strategy composability (**`validateComposability.ts`**), Tax tab stress, estate MC (lib + async + edge), projection death-year federal (**`estate-tax-projection.ts`**), ILIT/CST/alerts/narrative marginal savings. **`federalBrackets`** threaded through advisor shell → Strategy tab → **`MonteCarloPanel`** POST. Edge function must be redeployed after federal logic changes: **`supabase functions deploy estate-monte-carlo`**.
- **Advisor logo upload (2026-06-07):** **`POST/DELETE /api/advisor/profile/logo`** → Storage bucket **`advisor-branding`** → **`profiles.firm_logo_url`**. Settings UI at **`/advisor/settings`**. PDF cover reads existing **`firmLogoUrl`** path. Migration **`20260630120000_advisor_branding_storage.sql`**.
...
- **Retirement tab polish (2026-05-30):** `page.tsx` wires `scenarioOutputs` (`YearRow[]`), `loadSocialSecurityData(clientId)`, `runRothAnalysis()`; `RetirementTab.tsx` — readiness hero, retirement-year snapshot, SS survivor/breakeven, RMD timeline, Roth window, withdrawal sequencing; loaders enable `scenario` + `stateIncome` on retirement tab.
