...
- As of **2026-05-30**, consumer **`/dashboard`** estate summary collapsible: **no** Common Planning Topics list; titling conflicts show badge pills + link to `/titling` only (`conflictReport.critical` / `.warnings`). Bypass trust alert in **`EstateSummaryHeroAndMetrics.afterMetrics`** — after metric tiles, before checklist grid; **`parseBypassTrustSavings`** reads `by $…` from cached `bypass_trust` recommendation reason (Alan prod: **$645,463**).
