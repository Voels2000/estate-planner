...
- As of **2026-05-29 (Sprint B)**, consumer **`/dashboard`** uses **`getDashboardState()`** (2 | 3 in client; 1 = **`DashboardOnramp`** in `page.tsx`). **State 2:** net worth hero, source breakdown, financial metrics, estate readiness `{n}/100` tile + `/health-check` nudge, amber estate-unlock prompt, **`SetupProgressCard`**. **State 3:** tax exposure hero, adaptive score-band greeting, **`EstateReadinessCard`** (benchmark bar + component pills + trend delta), single **`PriorityAlertCard`** from open **`household_alerts`**, collapsible other items, checklist/tax snapshot. Data: `priorScore` + `openAlerts` in **`dashboard/_dashboard-body.tsx`**; helpers in **`lib/dashboard/scoreDisplayHelpers.ts`**.
