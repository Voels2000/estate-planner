...
- As of **2026-05-31**, **`ConsumerEstateFlowView`** (“What happens when I die?” on **`/my-estate-strategy`**) timeframe tabs drive **`computedGrossEstate`** via **`findClosestOutputRow`** / **`findAtDeathRow`** in **`generateEstateFlow.ts`**; owner pill and summary trust/probate scale with horizon; asset category tiles remain today's holdings with a projected-horizon context note.
