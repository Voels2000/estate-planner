...
- **Tax term explainers (Domain 3):** `lib/estate/taxTermExplainers.ts` — `taxTermExplainer(key, ctx?)` for educational tooltips (not compliance footers). Wired: **`EstateReadinessCard`** score subcategories; **`EstateSummaryHeroAndMetrics`** tile labels (`statePrimary` only at hero); consumer **`/estate-tax`** summary cards + waterfall (`taxTermCtx` from `primaryStateBrackets`; **`SummaryCard.labelTooltip`** sibling pattern). Pending: advisor **`StateTaxPanel`** badge pills + Exemption column header.
