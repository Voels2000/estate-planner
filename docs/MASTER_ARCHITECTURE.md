...
- **PDF narrative engine (2026-05-30):** `lib/export/narrativeEngine.ts` produces cover-page executive summary, tax callout (`clear` / `sunset_risk` / `exposed`), health trend, gifting bar, and themed enriched action items. Narrative inputs fetched in parallel via `lib/export/fetchNarrativePdfFields.ts`. **`lib/advisor/loadAdvisorExportWiring.ts`** centralizes export payload assembly for the client page and **`GET /api/advisor/meeting-prep-pdf/[clientId]?type=report`**. Header **Export estate report** and Meeting Prep **Export PDF Report** both call `generatePDFHTML`. Legacy one-page brief remains at **`?type=brief`**.
