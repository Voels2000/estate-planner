...
**Code paths:** `lib/billing/b2b2cBillingPolicy.ts` · `lib/billing/managedConsumerBilling.ts` · **`consumerCheckoutBlockReason()`** + **`processConsumerCheckout`** — shared UI/API gate on `POST /api/stripe/checkout` (PR #36; blocks managed, delinquent, active/trialing/canceling, connected advisor client). **Checkout return URLs:** `getOrigin(request)` in `lib/app-url.ts` (not `NEXT_PUBLIC_APP_URL`). **Stale `stripe_customer_id`:** retrieve-or-create self-heal in `processConsumerCheckout` (PR #94). Prod promotion smoke: **block paths only** (403/409); live eligible-consumer charge → real-card test ([PROMOTION_STAGING_TO_MAIN.md](./PROMOTION_STAGING_TO_MAIN.md)).
