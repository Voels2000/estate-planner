...
setup('authenticate plan-export purchaser consumer', async ({ page }) => {
  ...
  const password = resolveE2ePassword(
    email,
    process.env.PLAYWRIGHT_CONSUMER_PLAN_EXPORT_PASSWORD,
  )
  ...
  await page.context().storageState({ path: '.auth/consumer-plan-export.json' })
})
